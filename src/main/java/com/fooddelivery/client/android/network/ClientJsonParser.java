...
public class ClientJsonParser {
    ...
    public static ReplicatedStoreData parseReplicatedStoreData(String json) throws StoreJsonParser.JsonParseException {
        if (json == null || json.trim().isEmpty()) throw new StoreJsonParser.JsonParseException("ReplicatedStoreData JSON is null or empty.");
        
        String storeName = StoreJsonParser.extractStringValue(json, "storeName");
        if (storeName == null) throw new StoreJsonParser.JsonParseException("storeName missing in ReplicatedStoreData.");
        ReplicatedStoreData data = new ReplicatedStoreData(storeName);

        // Parse updatedProductStocks (Map<String, Integer>) - Complex manual parsing
        Pattern stocksPattern = Pattern.compile("\"updatedProductStocks\"\\s*:\\s*\\{([^\\}]*)\\}");
        Matcher stocksMatcher = stocksPattern.matcher(json);
        if (stocksMatcher.find()) {
            Map<String, Integer> stocks = new HashMap<>();
            String stocksJson = stocksMatcher.group(1);
            Pattern itemPattern = Pattern.compile("\"([^\"]+)\"\\s*:\\s*([0-9]+)");
            Matcher itemMatcher = itemPattern.matcher(stocksJson);
            while (itemMatcher.find()) {
                stocks.put(itemMatcher.group(1), Integer.parseInt(itemMatcher.group(2)));
            }
            if(!stocks.isEmpty()) data.setUpdatedProductStocks(stocks);
        }

        // Parse newSaleMade (SalesDataEntry)
        Pattern salePattern = Pattern.compile("\"newSaleMade\"\\s*:\\s*\\{([^\\}]*)\\}");
        Matcher saleMatcher = salePattern.matcher(json);
        if (saleMatcher.find()) {
            String saleJson = "{" + saleMatcher.group(1) + "}"; // Re-add braces for individual object parsing
            String itemName = StoreJsonParser.extractStringValue(saleJson, "itemName"); // productName
            int quantity = StoreJsonParser.extractIntValue(saleJson, "totalQuantity");
            double revenue = StoreJsonParser.extractDoubleValue(saleJson, "totalRevenue"); // priceAtTimeOfSale * quantity
            if(itemName != null) data.setNewSaleMade(new SalesDataEntry(itemName, quantity, revenue));
        }
        
        // Parse newRatingInfo
        Pattern ratingPattern = Pattern.compile("\"newRatingInfo\"\\s*:\\s*\\{([^\\}]*)\\}");
        Matcher ratingMatcher = ratingPattern.matcher(json);
        if (ratingMatcher.find()) {
            String ratingJson = "{" + ratingMatcher.group(1) + "}";
            int avgStars = StoreJsonParser.extractIntValue(ratingJson, "newAverageStars");
            int totalVotes = StoreJsonParser.extractIntValue(ratingJson, "newTotalVotes");
            data.setNewRating(avgStars, totalVotes);
        }
        return data;
    }

    public static PurchaseProcessingDetailsResponsePayload parsePurchaseProcessingDetailsResponse(String json) throws StoreJsonParser.JsonParseException {
        if (json == null || json.trim().isEmpty()) throw new StoreJsonParser.JsonParseException("PurchaseProcessingDetailsResponse JSON is null or empty.");
        String storeName = StoreJsonParser.extractStringValue(json, "storeName");
        String status = StoreJsonParser.extractStringValue(json, "status");
        String message = StoreJsonParser.extractStringValue(json, "message");
        if (storeName == null || status == null) throw new StoreJsonParser.JsonParseException("storeName or status missing.");
        ...
        SalesDataEntry saleRecorded = null;
        Pattern salePattern = Pattern.compile("\"saleRecorded\"\\s*:\\s*\\{([^\\}]*)\\}");
        Matcher saleMatcher = salePattern.matcher(json);
        if (saleMatcher.find()) {
            String saleJson = "{" + saleMatcher.group(1) + "}";
            String itemName = StoreJsonParser.extractStringValue(saleJson, "itemName");
            int quantity = StoreJsonParser.extractIntValue(saleJson, "totalQuantity");
            double revenue = StoreJsonParser.extractDoubleValue(saleJson, "totalRevenue");
            if(itemName != null) saleRecorded = new SalesDataEntry(itemName, quantity, revenue);
        }
        return new PurchaseProcessingDetailsResponsePayload(storeName, status, message, stocks, saleRecorded);
    }

    public static RatingProcessingDetailsResponsePayload parseRatingProcessingDetailsResponse(String json) throws StoreJsonParser.JsonParseException {
         if (json == null || json.trim().isEmpty()) throw new StoreJsonParser.JsonParseException("RatingProcessingDetailsResponse JSON is null or empty.");
        String storeName = StoreJsonParser.extractStringValue(json, "storeName");
        String status = StoreJsonParser.extractStringValue(json, "status");
        String message = StoreJsonParser.extractStringValue(json, "message");
        if (storeName == null || status == null) throw new StoreJsonParser.JsonParseException("storeName or status missing.");
        
        int avgStars = StoreJsonParser.extractIntValue(json, "newAverageStars"); // Directly from top-level if not nested in payload
        int totalVotes = StoreJsonParser.extractIntValue(json, "newTotalVotes"); // Directly from top-level
        return new RatingProcessingDetailsResponsePayload(storeName, status, message, avgStars, totalVotes);
    }
}
