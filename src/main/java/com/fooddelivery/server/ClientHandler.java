...
public class ClientHandler implements Runnable {
    ...
    public void run() {
        try {
            ...
            System.err.println("ClientHandler: Unexpected error for " + (type != null ? type : "UNKNOWN_TYPE") + " from " + clientSocket.getRemoteSocketAddress() + ": " + e.getMessage());
            // e.printStackTrace(); // Will be printed below
            if (out != null && !clientSocket.isClosed()) {
                 String errorDetails = e.getMessage() != null ? e.getMessage() : "No specific details.";
                 System.err.println("ClientHandler: Sending generic failure to client " + clientSocket.getRemoteSocketAddress() + " for " + (type != null ? type : "UNKNOWN_TYPE") + " after unhandled exception: " + errorDetails);
                 e.printStackTrace(); // Detailed log for server
                 out.println(JsonUtil.createStatusResponseJson(routingKey, "FAILURE", "Internal server error in Master processing " + (type != null ? type : "your request") + "."));
            }
        }
        finally {
            if (out != null && !clientSocket.isClosed() && out.checkError() && type == null) { // If type is null, no response has been sent
                // This case is tricky because we might have already sent a response if error occurred after initial handling
                // Only send generic if no other response was likely sent.
                 System.err.println("ClientHandler: No specific response generated for " + (type != null ? type : "UNKNOWN_TYPE") + ", sending generic failure to " + clientSocket.getRemoteSocketAddress());
                out.println(JsonUtil.createStatusResponseJson(routingKey, "FAILURE", "Master error processing request. Check server logs."));
            }
            closeConnection();
        }
    }
