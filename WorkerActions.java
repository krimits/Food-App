    ...
    public void run() {
        try {
            ...
            if (role.equals("manager")) {
                ...
                out.flush();
                // Προσθήκη για fetchLastUserPurchase
            } else if (role.equals("fetchLastUserPurchase")) {
                String email = (String) in.readObject();
                Purchase lastPurchase = null;
                synchronized (lock) {
                    for (Store store : stores) {
                        for (Purchase purchase : store.getPurchases()) {
                            if (purchase.getCustomerEmail().equalsIgnoreCase(email)) {
                                lastPurchase = purchase; // Κρατάει το τελευταίο που βρίσκει (αν δεν έχεις ημερομηνία)
                            }
                        }
                    }
                }
                out.writeObject(lastPurchase);
                out.flush();
                            ...
                            for (Purchase purchase : store.getPurchases()) {
                                if (purchase.getCustomerName().equalsIgnoreCase(customerName)) {
                                    for (Product product : purchase.getPurchasedProducts()) {
                                        purchases.put(
                                            product.getName(),
                                            purchases.getOrDefault(product.getName(), 0) + product.getQuantity()
                                        );
                                    }
                                }
                            }
